...
export class GraylogClient {
     ...
     */
    send(level, msg, topic, meta, serviceName, routerName) {
        if (!this.socket || !this.options.enabled)
            return;
        // console.log('META', meta);
        // Required fields
        const nowSec = Date.now() / 1000;
        const hostField = this.options.hostName || os.hostname();
        const shortMessage = typeof msg === 'string' && msg.length > 0
            ? msg
            : topic
                ? `${topic} ${String(level)}`
                : String(level);
    ...
    sendChunks(buf, id, total, chunkPayload) {
        let offset = 0;
