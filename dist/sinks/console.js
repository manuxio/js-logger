...
export function consoleWrite(devPretty, topic, level, msg, data) {
    let copyOfData = {};
    ...
    if (devPretty) {
        ...
        const topicColored = copyOfData && copyOfData.router && copyOfData.script ?
        ...
        if (copyOfData && typeof copyOfData === "object" && Object.keys(copyOfData).length > 0) {
            ...
            consoleMethodFor(level)(`${dateStr} ${topicColored} ${levelColored} ${nMsg}\n${rendered ? rendered + "\n" : ""}`);
        }
        else {
