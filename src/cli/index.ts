...
let globalMinLevel: LevelName | undefined;

function shouldDisplayLog(logLevel: LevelName): boolean {
  if (!globalMinLevel) return true;
    ...
    if (arg === '-f' || arg === '--file') {
      ...
      if (i < args.length) {
        ...
        if (['fatal', 'error', 'warn', 'info', 'debug', 'trace', 'verbose'].includes(level)) {
          options.minLevel = level as LevelName;
        } else {
...
async function tailFile(filePath: string, showLastLines: number = 10) {
  let position = 0;
...
async function readFromPosition(filePath: string, start: number) {
  return new Promise<void>((resolve, reject) => {
    const stream = createReadStream(filePath, {
      start,
      encoding: 'utf8'
...
async function readFromStream(stream: NodeJS.ReadableStream) {
  return new Promise<void>((resolve, reject) => {
    const rl = createInterface({
      input: stream,
      crlfDelay: Infinity
