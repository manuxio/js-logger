...
export function stdoutWrite(devPretty: boolean, topic: string, level: LevelName, msg: string, data: Record<string, unknown>): string | undefined {
  if (devPretty) {
    const dateStr = humanDate();
    const topicColored = 
      data && data.router && data.script ?
